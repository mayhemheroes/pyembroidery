...
class EnhancedFuzzedDataProvider(atheris.FuzzedDataProvider):
    ...
    def PickValueInList(self, values: List[Any]) -> Any:
        # Deterministically choose one element from a non-empty list using fuzzer bytes.
        # The original pyembroidery Mayhem harness called this method, but the shipped
        # fuzz_helpers.py never defined it — so every input raised AttributeError (caught
        # and swallowed) and the readers were never actually exercised. Defining it makes
        # the reader-fuzz target hit real pyembroidery code.
        if not values:
            return None
